...
### Lame Duck Financial Penalties

If the player loses their primary, the following changes take effect immediately:

- **Fundraiser yield reduced by 75%.** Base yield drops from $800 to $200 per event. Special interests and donors no longer see the player as a long-term investment. All fundraiser modifiers (party standing, faction ally bonus, weekly bonuses/penalties) still apply to the reduced base.
- **Report card payouts suspended entirely.** No weekly income from factions, party, or the President regardless of grade. These organizations redirect their money to the player's primary victor.
- **Banana Lobby exception:** If the player has an A+ grade with the Banana Lobby, the fundraiser yield penalty is only 25% instead of 75% (base yield $600 instead of $200). The Lobby pays for votes, not futures — a Lame Duck who still votes correctly is still useful to them.

The player's existing war chest is not confiscated. Whatever money they have on the day they lose becomes functionally all they'll have for the rest of the session, plus whatever pittance they can scrape from reduced fundraisers. This creates sharply different game states for Early Losers (Day 20, 40 days of Lame Duck with a thin war chest) versus Late Losers (Day 45, 15 days of Lame Duck with a full war chest they no longer need for campaigning).
...
### Lame Duck Status Modifiers

When the **player** has lost their primary and is operating as a Lame Duck, NPCs apply additional modifiers based on their temperament when evaluating the player's asks:

| Temperament | Lame Duck Modifier | Logic |
|---|---|---|
| **Ideologue** | Sentiment ×1.5 | Respects the player's freedom from electoral pressure. A Lame Duck who still votes on principle earns admiration. The relationship deepens because the player can no longer be accused of political calculation. |
| **Follower** | Sentiment ×0.5 | Still obeys the Whip at full strength — Party Pressure is unchanged. But the player's personal relationship counts for less. The Follower won't stick their neck out for someone who won't be around next session. When the Whip backs the player's bill, the Follower votes yes. When the Whip is silent, the player's friendship is worth half what it was. |
| **Dealmaker** | Flat -20 to Net Willingness | Trades in futures the player can no longer deliver. The relationship the player built is devalued because the Dealmaker can't collect on it next session. This penalty fights against even positive components — a Dealmaker who was at "I'm with you" may drop to "What's in it for me?" |
| **Opportunist** | Flat +15 to Net Willingness | The Lame Duck is the ultimate cheap date — 0 District Pressure means no electoral strings attached. The Opportunist sees a free vote to harvest for their own bills. Lame Ducks become more attractive to Opportunists, not less. |

These modifiers stack with the standard temperament multipliers. They apply only when the player is a Lame Duck, not when NPC Lame Ducks interact with the player (NPC Lame Ducks are handled by their District Pressure dropping to 0, which is already specified).

**The strategic hierarchy:** Opportunists become *easier* (they're predators, you're easy prey). Ideologues become *more respectful* (they admire your freedom). Followers stay *institutionally reliable* but personally distant (they follow the Whip, not you). Dealmakers become *much harder* (you have nothing to trade).
...
- **Player is a Lame Duck** (temperament-filtered — fires once, at the first meeting after the player loses their primary):
  - Ideologue: *"The voters were wrong about you. But your convictions are still right. Let's work."*
  - Follower: *"I'm sorry about your primary. But leadership's already looking ahead. I have to be careful."*
  - Dealmaker: *"You won't be here next session. That changes what you're worth to me."*
  - Opportunist: *"No district, no primary, no strings. That makes you very interesting to me."*
...
### Lame Duck Members (NPCs)

NPC members who lose their primary remain in congress until session's end. Their District Pressure component drops to 0. They have nothing to lose. Lame ducks are cheap allies — their endorsement is worthless but their vote still counts. NPCs whose primary meter is critical (above 70) abandon their bills to campaign; those who lose shift from active legislators to free agents available for vote trades.

### Lame Duck Player

If the player loses their primary, the game does not end. The player remains in session as a Lame Duck with the following consequences:

**Immediate effects:**
- District Pressure drops to 0 on all NPC evaluations of the player's asks
- Primary meter is no longer tracked (the player has already lost)
- Campaign back home is no longer available as an action (no election to campaign for)
- Report card payouts from all factions, party, and President are suspended immediately
- Fundraiser yield reduced by 75% (or 25% with Banana Lobby A+ grade)

**NPC temperament reactions** (see System 5 for full modifiers): Ideologues respect the player's freedom (+). Followers stay institutionally loyal but personally distant (-). Dealmakers devalue the relationship sharply (-). Opportunists see a cheap vote to harvest (+).

**The Dawn Brief displays a prominent notice on the day the primary is lost:**

```
YOUR PRIMARY IS LOST. You remain in session as a Lame Duck.
- Report card payouts suspended
- Fundraiser yields reduced by 75%
- Some members may distance themselves from you
Your bill is still alive. The session continues.
```

**The Early Loser (Day 20):** 40 days of free agent status. Zero District Pressure means legislative freedom — every vote is purely strategic. But the player is functionally broke: whatever money they saved in the first 20 days is most of what they'll have. The war chest becomes purely an influence tool (colleague fundraisers, party contributions) since campaign spending is no longer needed. The challenge is passing a bill with depleted resources and a political identity that's shifted — Dealmakers are hostile, but Opportunists are newly friendly, and Ideologues respect the player more than ever.

**The Late Loser (Day 45):** 15 days remaining. The financial hit barely matters — the player should have a full war chest by now and no longer needs campaign funds. But the Dealmaker -20 penalty and Follower sentiment halving hit right before the final floor vote push, potentially collapsing carefully built coalitions. The Late Loser's challenge isn't money — it's watching relationships devalue at the worst possible moment.

### Mid-Game Shift

After Day 20, the Early primary results transform the landscape. Some survived (free agents willing to deal). Some lost (unpredictable lame ducks with 0 District Pressure). Smart players plan around this shift — cultivating potential lame duck allies before the primary, then leveraging their freedom after it.
...
**Fundraiser Modifiers:** Insider ×1.3, Good Standing ×1.0, Thin Ice ×0.8, Outcast ×0.6, **Lame Duck ×0.25** (or ×0.75 with Banana Lobby A+). Faction ally (A+ or A) bonus: +$200. First fundraiser of week: +$100. Third+ fundraiser of week: -$200. Lame Duck: all report card payouts suspended.
...
| Total bark corpus | ~843 lines across 5 files |